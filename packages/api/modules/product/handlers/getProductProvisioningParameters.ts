...
export const handler = createHandler<APIGatewayProxyHandlerV2WithJWTAuthorizer>(async (event) => {
    ...
    const productIdPathParam = event.pathParameters?.productId;

    if (!productIdPathParam) {
        throw InvalidPathParamsError('productId');
    }

    const response = await productService.getProductProvisioningParameters(productIdPathParam);
