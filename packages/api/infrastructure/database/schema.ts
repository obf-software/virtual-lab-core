...
export const instanceConnectionType = pgEnum('instance_connection_type', ['VNC', 'RDP']);
...
export const instance = pgTable('instance', {
    ...
    userId: integer('user_id')
        .notNull()
        .references(() => user.id),
    logicalId: varchar('logical_id', { length: 50 }), // aws instance id, azure vm id, etc
    provisionToken: varchar('provision_token', { length: 50 }).unique().notNull(), // id that links to the product that provisioned this instance
    name: varchar('name', { length: 128 }).notNull(),
    description: text('description').notNull(),
